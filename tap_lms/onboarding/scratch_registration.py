...
def _get_glific_language_id(language_name):
    language_docname = _get_language_name_to_id(language_name)
    if language_docname:
        language_id = frappe.db.get_value(
            "TAP Language",
            language_docname,
...
def _enqueue_glific_contact_sync(doctype, docname):
    frappe.enqueue(
        "tap_lms.onboarding.scratch_registration.sync_registration_contact_to_glific",
        queue="short",
        timeout=60,
...
def sync_registration_contact_to_glific(doctype, docname, retry_count=0):
    try:
        ...
        ok = update_contact_fields(glific_id, fields_to_update, language_id=language_id)
        if not ok:
            raise RuntimeError(
