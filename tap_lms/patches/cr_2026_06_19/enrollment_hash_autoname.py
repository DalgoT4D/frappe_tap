"""CR-2026-06-19 review: convert the Student Enrollment child doctype from
counter-autoname to hash naming (parallel-worker contention fix).

`Student Enrollment` (Student's child table, `tabStudent Enrollment`) used
...
canonical student ID, referenced in Glific / CSV / displays). Student Enrollment can:
...
def execute():
    frappe.reload_doc("tap_lms", "doctype", "student_enrollment")
