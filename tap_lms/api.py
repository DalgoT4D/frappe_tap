...
import frappe
import requests
from frappe.utils import cint, cstr, get_datetime, get_url, getdate, now_datetime, today
...
def get_active_batch_for_school(school_id):
    ...
    if active_batch_onboardings:
        ...
        return {"batch_name": batch_name, "batch_id": batch_id}

    emit(severity="ERROR", message="active_batch_not_found", school_id=school_id)
...
def list_districts():
    try:
        ...
        districts = frappe.get_all(
            "District", filters={"state": state}, fields=["name", "district_name"]
        )

        response_data = {
            district.name: district.district_name for district in districts
        }
        ...
        emit(
            severity="ERROR",
            message="api_list_districts_exception",
            error=str(e),
...
def list_cities():
    try:
        ...
        cities = frappe.get_all(
            "City", filters={"district": district}, fields=["name", "city_name"]
        )
        ...
        emit(
            severity="ERROR",
            message="api_list_cities_exception",
            error=str(e),
            traceback=traceback.format_exc(),
        )
        frappe.response.http_status_code = 500
...
def send_whatsapp_message(phone_number, message):
    ...
    if not gupshup_settings:
        ...
        emit(severity="ERROR", message="gupshup_settings_missing")
        return False

    if not all(
        [
            gupshup_settings.api_key,
            gupshup_settings.source_number,
            gupshup_settings.app_name,
            gupshup_settings.api_endpoint,
        ]
    ):
        frappe.log_error("Incomplete Gupshup OTP Settings")
        emit(severity="ERROR", message="gupshup_settings_incomplete")
        return False
...
def verify_keyword():
    ...
    school = frappe.db.get_value(
        "School", {"keyword": keyword}, ["name1", "model"], as_dict=True
    )

    if school:
        frappe.response.http_status_code = 200
        frappe.response.update(
            {"status": "success", "school_name": school.name1, "model": school.model}
        )
    else:
        frappe.response.http_status_code = 404
        frappe.response.update(
            {"status": "failure", "school_name": None, "model": None}
        )
...
def create_teacher(
    ...
    try:
        ...
        if not school:
            return {"error": f"No school found with the keyword: {keyword}"}
        ...
        return {"message": "Teacher created successfully", "teacher_id": teacher.name}
    except frappe.DuplicateEntryError:
        return {"error": "Teacher with the same phone number already exists"}
    except Exception as e:
        return {"error": f"An error occurred while creating teacher: {str(e)}"}
...
def list_batch_keyword(api_key):
    ...
    batch_onboarding_list = frappe.get_all(
        "Batch onboarding", fields=["batch", "school", "batch_skeyword"]
    )
...
def create_student():
    try:
        ...
        if batch_doc.regist_end_date:
            try:
                ...
                if regist_end_date < current_date:
                    ...
                    return {
                        "status": "error",
                        "message": "Registration for this batch has ended",
                    }
            except Exception as e:
        ...
        course_vertical = frappe.get_all(
            "Course Verticals", filters={"name2": vertical}, fields=["name"]
        )
        ...
        if existing_student:
            ...
            # Create new student
            student = create_new_student(
                student_name, phone, gender, school_id, grade, language_name, glific_id
            )
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Course selection failed: {str(course_error)}",
            }
...
def get_course_level_with_mapping(
    course_vertical, grade, phone_number, student_name, kitless
):
    """
    ...
    try:
        ...
        student_type = determine_student_type(
            phone_number, student_name, course_vertical
        )
...
def get_course_level_original(course_vertical, grade, kitless):
    ...
    try:
        ...
        if not course_level and kitless:
            ...
            course_level = frappe.get_all(
                "Course Level",
                filters={"vertical": course_vertical, "stage": stage[0].name},
                fields=["name"],
                order_by="modified desc",
                limit=1,
            )
...
def get_tap_language(language_name):
    tap_language = frappe.get_all(
        "TAP Language", filters={"language_name": language_name}, fields=["name"]
    )
...
def verify_batch_keyword():
    try:
        ...
        if not api_key or not batch_skeyword:
            ...
            return {
                "status": "error",
                "message": "API key and batch_skeyword are required",
            }
        ...
        school_name = cstr(
            frappe.get_value("School", batch_onboarding[0].school, "name1")
        )
        batch_id = cstr(
            frappe.get_value("Batch", batch_onboarding[0].batch, "batch_id")
        )
...
def course_vertical_list():
    try:
        ...
        batch_onboarding = frappe.get_all(
            "Batch onboarding", filters={"batch_skeyword": keyword}, fields=["name"]
        )
        ...
        for vertical in batch_school_verticals:
            course_vertical = frappe.get_doc(
                "Course Verticals", vertical.course_vertical
            )
...
def list_schools():
    ...
    if not data or "api_key" not in data or not authenticate_api_key(data["api_key"]):
        frappe.response.http_status_code = 401
        frappe.response.update(
            {"status": "failure", "schools": [], "error": "Invalid API key"}
        )
        return
    ...
    if schools:
        frappe.response.http_status_code = 200
        frappe.response.update({"status": "success", "schools": schools})
    else:
        frappe.response.http_status_code = 404
        frappe.response.update(
            {
...
def send_otp_gs():
    ...
    existing_teacher = frappe.get_all(
        "Teacher", filters={"phone_number": phone_number}, fields=["name"]
    )
    if existing_teacher:
...
def send_otp_v0():
    ...
    whatsapp_api_key = (
        "J3tuS4rCqzcLiqt2SjyeiqYxjVLICnWb"  # Replace with your actual API key
    )
...
def send_otp():
    try:
        ...
        if (
            ...
            or not authenticate_api_key(data["api_key"])
        ):
            frappe.response.http_status_code = 401
            return {"status": "failure", "message": "Invalid API key"}
        ...
        if existing_teacher:
            ...
            if (
                not batch_info["batch_id"]
                or batch_info["batch_id"] == "no_active_batch_id"
            ):
                frappe.response.http_status_code = 409
                return {
                    "status": "failure",
            ...
            if existing_group_mapping:
                ...
                teacher_glific_id = frappe.db.get_value(
                    "Teacher", teacher.name, "glific_id"
                )
        ...
        # Send WhatsApp message using the API
        whatsapp_api_key = frappe.conf.get(
            "whatsapp_api_key", "J3tuS4rCqzcLiqt2SjyeiqYxjVLICnWb"
        )
        ...
        try:
            ...
            if response_data.get("status") == "success":
                ...
                frappe.log_error(
                    f"WhatsApp API Error: {response_data.get('message')}",
                    "WhatsApp API Error",
                )
                frappe.response.http_status_code = 500
                return {
            ...
            frappe.log_error(
                f"WhatsApp API Request Error: {str(e)}", "WhatsApp API Request Error"
            )
            frappe.response.http_status_code = 500
            return {
                "status": "failure",
        ...
        if action_type == "update_batch":
            try:
                ...
                if not teacher.glific_id:
                    emit(
                        severity="WARNING",
                        message="teacher_missing_glific_id",
                    ...
                    if glific_contact and "id" in glific_contact:
                        ...
                        emit(
                            severity="INFO",
                            message="teacher_linked_to_glific",
                            teacher_id=teacher_id,
                        ...
                        if teacher.language:
                            language_id = frappe.db.get_value(
                                "TAP Language", teacher.language, "glific_language_id"
                            )
                        ...
                        if not language_id:
                            emit(
                                severity="WARNING",
                                message="no_english_language_found",
                                teacher_id=teacher_id,
                            )
                            language_id = None
                        ...
                        try:
                            new_contact = create_contact(
                                teacher.first_name
                                or "Teacher",  # Fallback if first_name is empty
                        ...
                        if new_contact and "id" in new_contact:
                            ...
                            emit(
                                severity="INFO",
                                message="teacher_glific_contact_created",
                                teacher_id=teacher_id,
                                glific_id=new_contact["id"],
                            )
                        else:
                            emit(
                                severity="ERROR",
                                message="teacher_glific_contact_creation_failed",
                                teacher_id=teacher_id,
                            )
                ...
                if teacher.glific_id:
                    ...
                    update_success = update_contact_fields(
                        teacher.glific_id, fields_to_update
                    )

                    if not update_success:
                        emit(
                            severity="WARNING",
                            message="glific_contact_fields_update_failed",
                            teacher_id=teacher_id,
                    ...
                    emit(
                        severity="WARNING",
                        message="teacher_still_missing_glific_id",
                        teacher_id=teacher_id,
                    )
                ...
                if teacher.glific_id:
                    teacher_group = create_or_get_teacher_group_for_batch(
                        batch_info["batch_name"], batch_info["batch_id"]
                    )

                    if teacher_group:
                        try:
                            group_added = add_contact_to_group(
                                teacher.glific_id, teacher_group["group_id"]
                            )
                        except requests.exceptions.RequestException as e:
                            frappe.log_error(
                                title="Glific timeout (degraded)",
                        ...
                        if group_added:
                            emit(
                                severity="INFO",
                                message="teacher_added_to_batch_group",
                                teacher_id=teacher_id,
                                glific_id=teacher.glific_id,
                            ...
                            emit(
                                severity="WARNING",
                                message="teacher_group_addition_failed",
                                teacher_id=teacher_id,
                                glific_id=teacher.glific_id,
                            )
                ...
                try:
                    ...
                    emit(
                        severity="WARNING",
                        message="teacher_batch_history_creation_failed",
                        teacher_id=teacher_id,
                        error=str(e),
                    )
                ...
                emit(
                    severity="ERROR",
                    message="teacher_batch_update_exception",
                    error=str(e),
                    traceback=traceback.format_exc(),
                    teacher_id=teacher_id,
                    school_id=school_id,
                ...
                frappe.log_error(
                    f"Error updating teacher batch in verify_otp: {str(e)}",
                    "Teacher Batch Update Error",
                )
                frappe.response.http_status_code = 500
                return {
                    "status": "failure",
...
def create_teacher_web():
    try:
        ...
        verification = frappe.db.get_value(
            "OTP Verification", {"phone_number": data["phone"], "verified": 1}, "name"
        )
        if not verification:
            return {
                "status": "failure",
        ...
        existing_teacher = frappe.db.get_value(
            "Teacher", {"phone_number": data["phone"]}, "name"
        )
        if existing_teacher:
            return {
                "status": "failure",
        ...
        language_id = frappe.db.get_value(
            "TAP Language", data.get("language"), "glific_language_id"
        )
        if not language_id:
            language_id = frappe.db.get_value(
                "TAP Language", {"language_name": "English"}, "glific_language_id"
        ...
        if not batch_id:
            emit(
                severity="WARNING",
                message="active_batch_not_found_create_teacher",
                school_id=school,
            )
        ...
        if glific_contact and "id" in glific_contact:
            ...
            emit(
                severity="INFO",
                message="existing_glific_contact_found",
                glific_id=glific_contact["id"],
            ...
            update_success = update_contact_fields(
                glific_contact["id"], fields_to_update
            )
        ...
        if not (glific_contact and "id" in glific_contact):
            ...
            emit(
                severity="INFO",
                message="creating_new_glific_contact_teacher",
                teacher_id=new_teacher.name,
                phone=data.get("phone"),
            )
            try:
        ...
        emit(
            severity="ERROR",
            message="create_teacher_web_exception",
            error=str(e),
            traceback=traceback.format_exc(),
        )
        return {"status": "failure", "message": f"Error creating teacher: {str(e)}"}
    finally:
...
def get_course_level(course_vertical, grade, kitless):
    frappe.log_error(
        f"Input values: course_vertical={course_vertical}, grade={grade}, kitless={kitless}"
    )
    ...
    if not stage:
        ...
        """
        frappe.log_error(
            f"Specific stage query: {query[:100]}..." if len(query) > 100 else query
        )
        stage = frappe.db.sql(query, (grade, grade), as_dict=True)
    ...
    frappe.log_error(
        f"Course level query filters: vertical={course_vertical}, stage={stage[0].name}, kit_less={kitless}"
    )
    ...
    if not course_level and kitless:
        ...
        frappe.log_error(
            f"Fallback course level query filters: vertical={course_vertical}, stage={stage[0].name}"
        )
...
def get_course_level_api():
    try:
        ...
        return {"status": "success", "course_level": course_level}
...
def get_model_for_school(school_id):
    ...
    if active_batch_onboardings:
        ...
        emit(
            severity="INFO",
            message="model_resolved_from_batch_onboarding",
            school_id=school_id,
            creation=str(active_batch_onboardings[0].creation),
        )
    else:
        ...
        emit(
            severity="INFO",
            message="model_resolved_from_school_default",
            school_id=school_id,
        )
...
def update_teacher_role():
    ...
    try:
        ...
        school_name = (
            frappe.db.get_value("School", teacher_doc.school_id, "name1")
            if teacher_doc.school_id
            else None
        )
        ...
        frappe.log_error(
            f"Update Teacher Role Error: {str(e)}", "Update Teacher Role API"
        )
        frappe.response.http_status_code = 500
        return {
            "status": "error",
...
def get_teacher_by_glific_id():
    ...
    try:
        ...
        # Get related data
        school_name = (
            frappe.db.get_value("School", teacher_data.school_id, "name1")
            if teacher_data.school_id
            else None
        )
        language_name = (
            frappe.db.get_value("TAP Language", teacher_data.language, "language_name")
            if teacher_data.language
            else None
        )
        course_level_name = (
            frappe.db.get_value("Course Level", teacher_data.course_level, "name1")
            if teacher_data.course_level
            else None
        )
        ...
        return {
            ...
            "data": {
                ...
                "school": {"id": teacher_data.school_id, "name": school_name},
                "language": {"id": teacher_data.language, "name": language_name},
                "course_level": {
...
def get_school_city():
    ...
    try:
        ...
        if not school_data.city:
            ...
            if school_data.country:
                country_name = frappe.db.get_value(
                    "Country", school_data.country, "country_name"
                )
            ...
            if school_data.state:
                state_name = frappe.db.get_value(
                    "State", school_data.state, "state_name"
                )
        ...
        frappe.log_error(
            f"Get School City Error - Document not found: {str(e)}",
            "Get School City API",
        )
        frappe.response.http_status_code = 404
        return {
...
def search_schools_by_city():
    ...
    try:
        ...
        frappe.log_error(
            f"Search Schools by City Error: {str(e)}", "Search Schools API"
        )
        frappe.response.http_status_code = 500
        return {
            "status": "error",
...
def glific_list_grades(vertical: str = None):
    ...
    for cl in course_levels:
        ...
        try:
            from_grades = [
                int(x.strip()) for x in str(from_grade_raw).split(",") if x.strip()
            ]
        ...
        results.append({"label": f"Grade {grade_range}", "value": cl.name1 or cl.name})
...
@frappe.whitelist(allow_guest=True)
def get_youtube_url(
    api_key, course_vertical, phone_number, language, grade, batch_keyword=None
):
    """
    ...
    try:
        ...
        if not course_level:
            return error_response(
                _(
                    "No course level mapping found for Course Vertical: {0} and Grade: {1}"
                ).format(course_vertical, grade)
            )
        ...
        if batch_keyword:
            try:
                current_week = get_current_week_from_teacher_flow(
        ...
        if not video_class_name:
            return error_response(
                _("No VideoClass content found in learning unit {0}").format(
                    learning_unit
                )
            )
...
def get_current_week_from_teacher_flow(phone_number, batch_keyword):
    ...
    try:
        ...
        for flow in teacher_flows:
            ...
            if hasattr(flow_doc, "teachers"):
                for teacher in flow_doc.teachers:
                    if (
                        teacher.phone_number == phone_number
                        and teacher.batch_keyword == batch_keyword
                    ):
        ...
        raise frappe.DoesNotExistError(
            _(
                "Teacher record not found for phone number '{0}' and batch keyword '{1}'"
            ).format(phone_number, batch_keyword)
        )
        ...
        frappe.log_error(
            frappe.get_traceback(), _("Get Current Week from Teacher Flow Error")
        )
...
def calculate_current_week(batch_keyword):
    ...
    try:
        ...
        if not batch_onboarding:
            raise frappe.DoesNotExistError(
                _("Batch Onboarding with keyword '{0}' not found").format(batch_keyword)
            )
        ...
        if not batch_data or not batch_data.regular_activity_start_date:
            raise frappe.DoesNotExistError(
                _(
                    "Batch '{0}' not found or Regular Activity Start Date not set"
                ).format(batch_name)
            )
    ...
    try:
        ...
        for lu in course_level_doc.learning_units:
            learning_units.append(
                {"learning_unit": lu.learning_unit, "week_no": cint(lu.week_no)}
            )
...
def get_youtube_url_from_video_class(video_class_name, language):
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(), _("Get YouTube URL from VideoClass Error")
        )
        return None
...
def get_student_by_phone_and_name(api_key, phone_number, buddy_name):
    ...
    try:
        ...
        students = frappe.get_all(
            "Student",
            filters=[["name1", "=", buddy_name], ["phone", "in", [phone_10, phone_12]]],
        ...
        if latest_enrollment and latest_enrollment.batch:
            try:
                ...
                batch_active = (
                    batch_doc.active if hasattr(batch_doc, "active") else None
                )
            except:
                pass
        ...
        return {
            ...
            "data": {
                ...
                "batch_active": bool(batch_active)
                if batch_active is not None
                else None,
                # "current_grade": student.grade,
                "enrollment_grade": latest_enrollment.grade
                if latest_enrollment
                else None,
                "enrollment_school": latest_enrollment.school
                if latest_enrollment
                else None,
                "date_joining": str(latest_enrollment.date_joining)
                if latest_enrollment and latest_enrollment.date_joining
                else None,
                "status": student.status,
            },
        }
            ...
            for enrollment in student_doc.enrollment:
                ...
                if enrollment.batch:
                    try:
                        ...
                        batch_name = (
                            batch_doc.name1 if hasattr(batch_doc, "name1") else None
                        )
                        batch_active = (
                            batch_doc.active if hasattr(batch_doc, "active") else None
                        )
                        batch_start_date = (
                            str(batch_doc.start_date)
                            if hasattr(batch_doc, "start_date") and batch_doc.start_date
                            else None
                        )
                        batch_end_date = (
                            str(batch_doc.end_date)
                            if hasattr(batch_doc, "end_date") and batch_doc.end_date
                            else None
                        )
                    except:
                        pass
                ...
                enrollment_list.append(
                    {
                        ...
                        "date_joining": str(enrollment.date_joining)
...
@frappe.whitelist(allow_guest=True)
def get_student_video_content(
    api_key, course_level, batch, student_id, language, content_type
):
    """
        ...
        for learning_unit in learning_units:
            ...
            for video_class in video_classes:
                ...
                video_info = get_video_with_translation(
                    video_class, language, content_type
                )
...
def get_student_current_week(student_id, batch):
    ...
    try:
        ...
        weekly_flows = frappe.get_all(
            "Weekly Student Flow",
            filters=[["week_end_date", ">=", current_date]],
            fields=["name", "week_start_date", "week_end_date"],
            limit=1,
        )
        ...
        frappe.log_error(
            f"Error getting current week: {str(e)}", "Student Video Content API"
        )
        return None
...
def get_learning_units_by_week(course_level, week_no):
    ...
    try:
        ...
        frappe.log_error(
            f"Error getting learning units: {str(e)}", "Student Video Content API"
        )
        return []
...
@frappe.whitelist(allow_guest=False)
def get_student_all_week_videos(
    course_level, batch, student_id, language, content_type
):
    """
...
def get_quiz_by_id(api_key, student_id, quiz_id, language=None):
    ...
    try:
        ...
        if not student_id or not quiz_id:
            return {"success": False, "error": "student_id and quiz_id are required"}
        ...
        response = {"success": True, "quiz_count": 1}
...
def _get_quiz_with_questions_flat(quiz_id: str, language: str) -> dict:
    ...
    try:
        ...
        if hasattr(quiz_doc, "questions") and quiz_doc.questions:
            for question_row in quiz_doc.questions:
                ...
                question_number = (
                    ...
                    if hasattr(question_row, "question_number")
                    else question_row.idx
                )
        ...
        frappe.log_error(
            f"Error getting quiz with questions: {str(e)}", "Student Progression API"
        )
        return None
...
def _get_question_text_translated(question_id: str, language: str) -> str:
    ...
    try:
        ...
        if (
            language
            and hasattr(question_doc, "question_translations")
            and question_doc.question_translations
        ):
            for translation in question_doc.question_translations:
                if translation.language == language and translation.translated_question:
                    question_text = translation.translated_question
        ...
        frappe.log_error(
            f"Error getting question text: {str(e)}", "Student Progression API"
        )
        return None
...
def get_learning_units_by_week(course_level, week_no):
    ...
    try:
        ...
        frappe.log_error(
            f"Error getting learning units: {str(e)}", "Student Quiz Content API"
        )
        return []
...
def get_quizzes_from_unit(learning_unit_id):
    ...
    try:
        ...
        frappe.log_error(
            f"Error getting quizzes from unit: {str(e)}", "Student Quiz Content API"
        )
        return []
...
def get_quiz_with_questions(quiz_id, language):
    ...
    try:
        ...
        frappe.log_error(
            f"Error getting quiz with questions: {str(e)}", "Student Quiz Content API"
        )
        return None
...
def get_question_text(question_id, language):
    ...
    try:
        ...
        # Try to find translation for the specified language
        if (
            hasattr(question_doc, "question_translations")
            and question_doc.question_translations
        ):
            for translation in question_doc.question_translations:
                if translation.language == language and translation.translated_question:
        ...
        frappe.log_error(
            f"Error getting question text: {str(e)}", "Student Quiz Content API"
        )
        return None
...
def get_quiz_question_details(api_key, question_id, language):
    ...
    try:
        ...
        return {
            "message": {"success": False, "error": f"Internal server error: {str(e)}"}
        }
...
def get_options_by_language(question_doc, language):
    ...
    for option_row in question_doc.options:
        ...
        try:
            ...
            option_data = {
                "option_id": option_id,
                "option_number": order_number
                if order_number
                else option_doc.option_number,
                "option_text": option_text,
            }
            ...
            frappe.log_error(
                f"Error getting option details: {str(e)}", "Quiz Question Details API"
            )
            continue
...
def get_option_translation(option_doc, language):
    ...
    """
    if (
        not hasattr(option_doc, "option_translations")
        or not option_doc.option_translations
    ):
        return None
...
def get_multiple_questions(question_ids, language):
    ...
    try:
        # Validate language
        if not language:
            return {"success": False, "error": "language parameter is required"}
    ...
    try:
        ...
        if teacher.get("teacher_batch"):
            ...
            batch_details = {
                ...
                "engagement_end_date": batch.engagement_end_dae
                if hasattr(batch, "engagement_end_dae")
                else None,
                "regular_activity_start_date": batch.regular_activity_start_date
                if hasattr(batch, "regular_activity_start_date")
                else None,
            }
            ...
            response.update(
                {
                    ...
                    "regular_activity_start_date": batch_details[
                        "regular_activity_start_date"
                    ],
                }
            )
        ...
        frappe.log_error(
            f"Error in get_teacher_by_phone: {str(e)}", "Teacher Batch API Error"
        )
